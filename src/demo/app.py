...
celeb_information = loaddata.get_celeb_data("./data/filtered_celebs_data/celebs_200_9_clips")
...
def findmatch():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            filterd_dev = celeb_information[celeb_information['VoxCeleb2_ID'].isin(metadata[0])]

            order_df = pd.DataFrame({'VoxCeleb2_ID': metadata[0], 'order': range(len(metadata[0]))})

            filterd_dev = pd.merge(filterd_dev, order_df, on='VoxCeleb2_ID').sort_values('order').drop('order', axis=1)
